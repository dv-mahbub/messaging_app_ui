...
class SlideSwitcher extends StatefulWidget {
  ...
  ///Ability to tap on the current slider and change its index to the opposite (available only for 2 text)
...
class _SlideSwitcherState extends State<SlideSwitcher>
  ...
  void initState() {
    verticalBadSize = widget.direction == Axis.vertical &&
        widget.containerHeight < widget.containerWight * widget.text.length;

    horizontalBadSize = widget.direction == Axis.horizontal &&
        widget.containerHeight * widget.text.length > widget.containerWight;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Container(
          height: widget.text.isEmpty ? 0 : widget.containerHeight,
          width: widget.text.isEmpty ? 0 : widget.containerWight,
              ...
              Flex(
                ...
                children: List.generate(
                  widget.text.length,
                    ...
                    onTap: () {
                      _slidersOnTap(
                        widget.isAllContainerTap && widget.text.length == 2
                            ? 1 - index
                            : slidersIndex,
                      );
                    },
                    ...
                    child: SizedBox(
                      ...
                      child: SizedBox(
                        child: Center(
                          child: Text(widget.text[slidersIndex], style: TextStyle(color: themeNotifier.currentTheme==ThemeData.light() && index!=slidersIndex? Colors.black54:Colors.white),),
                        ),
                      ),
                    ),
                  ),
  ...
  BorderRadius _makingBorder(int index) {
    return BorderRadius.only(
      bottomLeft: index == 0 && verticalBadSize ||
          index == widget.text.length - 1 && horizontalBadSize
          ? Radius.zero
          : Radius.circular(sliderBorderRadius),
      bottomRight: index == 0 && (verticalBadSize || horizontalBadSize)
          ? Radius.zero
          : Radius.circular(sliderBorderRadius),
      topLeft: index == widget.text.length - 1 && verticalBadSize ||
          index == widget.text.length - 1 && horizontalBadSize
          ? Radius.zero
          : Radius.circular(sliderBorderRadius),
      topRight: index == widget.text.length - 1 && verticalBadSize ||
  ...
  void _slidersOnSwipe(DragEndDetails details) {
    if (details.primaryVelocity! > 0 && index != widget.text.length - 1) {
  ...
  void _checkForExceptions() {
    if (widget.isAllContainerTap && widget.text.length != 2) {
      debugPrint(
          '\x1B[31mThe "isAllContainerTap" parameter can be "true" only when "text" length is 2\nRemove "isAllContainerTap" or make 2 "text"\x1B[0m');
      throw 'The "isAllContainerTap" parameter can be "true" only when "text" length is 2\nRemove "isAllContainerTap" or make 2 "text"';
    }
    ...
    double longSide = widget.containerHeight > widget.containerWight
        ? widget.containerHeight
        : widget.containerWight;
    if (shortSide * widget.text.length * 0.5 > longSide &&
        widget.containerBorderRadius * 2 > shortSide &&
        widget.text.length != 2) {
      debugPrint(
          '\x1B[31mAll widgets from the list of "text" do not fit into the given container size.\nTry applying other container sizes\x1B[0m');
      throw 'All widgets from the list of "text" do not fit into the given container size.\nTry applying other container sizes';
    }
    lastIndex = 0;
  }
}